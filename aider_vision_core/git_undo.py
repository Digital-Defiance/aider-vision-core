"""
Undo a single aider commit in one git repository.
"""
...
def undo_commit_in_repo(git_repo: GitRepo, io, expected_hash: str | None = None) -> bool:
    """
    Undo the HEAD commit in ``git_repo`` if it matches ``expected_hash`` (short or full).

    Returns True when the commit was removed.
    """
...
def undo_last_aider_commit_for_coder(coder, io):
    """Undo the last aider commit batch for any repository layout."""
    from aider_vision_core import prompts
    ...
    if coder.aider_commit_stack:
        ...
        for entry in reversed(batch):
            if entry["hash"] not in coder.aider_commit_hashes:
            ...
            if not undo_commit_in_repo(repo, io, expected_hash=entry["hash"]):
                return
            coder.aider_commit_hashes.discard(entry["hash"])
