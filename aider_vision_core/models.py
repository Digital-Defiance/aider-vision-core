...
from aider_vision_core.openrouter import OpenRouterModelManager
from aider_vision_core.sendchat import ensure_alternating_roles, sanity_check_messages
from aider_vision_core.utils import check_pip_install_extra
...
MODEL_SETTINGS = []
with importlib.resources.open_text("aider_vision_core.resources", "model-settings.yml") as f:
...
class Model(ModelSettings):
    def __init__(
        ...
        self.extra_model_settings = next(
            (ms for ms in MODEL_SETTINGS if ms.name == "aider_vision_core/extra_params"), None
        )
    ...
    def configure_model_settings(self, model):
        ...
        if (
            ...
            and self.extra_model_settings.name == "aider_vision_core/extra_params"
        ):
        ...
        if "GITHUB_COPILOT_TOKEN" in os.environ:
            if "extra_headers" not in kwargs:
                kwargs["extra_headers"] = {
                    "Editor-Version": f"aider_vision_core/{__version__}",
    ...
    def simple_send_with_retries(self, messages):
        ...
        while True:
            try:
                ...
                from aider_vision_core.reasoning_tags import remove_reasoning_content
