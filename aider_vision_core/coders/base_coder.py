...
from aider_vision_core.history import ChatSummary
from aider_vision_core.io import ConfirmGroup, InputOutput
from aider_vision_core.linter import Linter
from aider_vision_core.llm import litellm
from aider_vision_core.models import RETRY_TIMEOUT
...
from aider_vision_core.git_workspace import create_git_workspace, primary_head_from_snapshot
from aider_vision_core.repo import ANY_GIT_ERROR, GitRepo
from aider_vision_core.repomap import RepoMap
from aider_vision_core.run_cmd import run_cmd
from aider_vision_core.utils import format_content, format_messages, format_tokens, is_image_file
from aider_vision_core.waiting import WaitingSpinner
...
class Coder:
    ...
    def create(
        ...
        **kwargs,
        ...
        import aider_vision_core.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                aider_commit_stack=list(from_coder.aider_commit_stack),
    ...
    def __init__(
        ...
        if aider_commit_hashes:
            ...
            self.aider_commit_hashes = set()
        if aider_commit_stack is not None:
            self.aider_commit_stack = aider_commit_stack
        else:
            self.aider_commit_stack = []
        ...
        if use_git and self.repo is None:
            try:
                self.repo = create_git_workspace(
                    self.io,
                    fnames,
                    None,
    ...
    def init_before_message(self):
        ...
        if self.repo:
            snap = (
                self.repo.get_commit_snapshot()
                if hasattr(self.repo, "get_commit_snapshot")
                else self.repo.get_head_commit_sha()
            )
            self.commit_before_message.append(snap)
    ...
    def show_auto_commit_outcome(self, res):
        batch = getattr(self.repo, "last_commit_batch", None) if self.repo else None
        if batch:
            self.aider_commit_stack.append(batch)
            for entry in batch:
                self.aider_commit_hashes.add(entry["hash"])
            last = batch[-1]
            self.last_aider_commit_hash = last["hash"]
            self.last_aider_commit_message = last["message"]
        elif res:
            commit_hash, commit_message = res
            self.aider_commit_stack.append(
                [{"root": self.repo.root, "hash": commit_hash, "message": commit_message}]
            )
    ...
    def show_undo_hint(self):
        if not self.commit_before_message:
            return
        prev = self.commit_before_message[-1]
        current = (
            self.repo.get_commit_snapshot()
            if hasattr(self.repo, "get_commit_snapshot")
            else self.repo.get_head_commit_sha()
