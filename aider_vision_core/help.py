...
from aider_vision_core import __version__, utils
from aider_vision_core.dump import dump  # noqa: F401
from aider_vision_core.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("aider_vision_core.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def get_index():
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("aider_vision_core.website")
                .joinpath(fname)
